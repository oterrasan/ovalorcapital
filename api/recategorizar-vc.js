...
export default async function handler(req, res) {
  ...
  // Resposta HTML para requisicoes GET (browser)
  if (req.method === 'GET') {
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    const titulo = dryRun
      ? `PRE-VISUALIZACAO -- ${result.total_encontrados} artigos encontrados com categoria VC/Colunistas`
      : `CORRECAO EXECUTADA -- ${result.total_recategorizados} artigos recategorizados`;
    ...
    const btnExecutar = dryRun
      ? `<p style="margin-top:20px"><a href="/api/recategorizar-vc?executar=1" style="background:#c00;color:#fff;padding:12px 24px;text-decoration:none;border-radius:4px;font-size:16px">EXECUTAR CORRECAO AGORA</a></p>`
      : `<p style="margin-top:20px;color:#060;font-size:18px">Correcao aplicada com sucesso! <a href="/vc/">Ver pagina VC</a></p>`;
...
<h2>${titulo}</h2>
${btnExecutar}
<table><thead><tr><th>ID</th><th>De</th><th></th><th>Para</th><th>Subcategoria</th><th>Titulo</th></tr></thead>
<tbody>${linhas}</tbody>
</table>
${btnExecutar}
</body></html>`);
