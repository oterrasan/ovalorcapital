...
export default async function handler(req, res) {
  ...
  const data = await loadMany(['articles','banners','site-settings','newsletter','alerts','reminders','campaigns','integrations','social-posts','live-config','social-templates','users','leads','routines','bulk-jobs','reports']);
  const stats = {
    articles: data['articles']?.length || 0,
    banners: data['banners']?.filter(item => item.active).length || 0,
    leads: (data['newsletter']?.length || 0) + (data['leads']?.length || 0),
    alerts: data['alerts']?.filter(item => item.active !== false).length || 0,
