...
export default async function handler(req, res) {
  ...
  // POST — add, mark done, or mark error
  if (req.method === "POST") {
    ...
    // Default: add post to queue
    const { post_id, titulo, imagem, resumo, categoria } = body;
    if (!post_id || !titulo || !imagem)
      return res.status(400).json({ error: "post_id, titulo e imagem são obrigatórios" });
