...
// Critical CSS: variables + reset inlined to prevent theme flash on first paint
...
export default async function handler(req, res) {
  ...
  // Step 1: Inline critical CSS before the CSS links to prevent theme flash
  html = html.replace(
    '<link href="/css/home.css?v=2" rel="stylesheet"/>',
    `<style>${CRITICAL_CSS}</style><link href="/css/home.css?v=2" rel="stylesheet"/>`
  );

  // Step 2: Pre-render hero content server-side (eliminates JS→API→LCP chain)
  try {
