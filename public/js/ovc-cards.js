(function(){
  ...
  var STYLE = {
    card: 'position:relative;border-radius:12px;overflow:hidden;min-height:220px;display:flex;flex-direction:column;justify-content:flex-end;padding:16px;background:#0f172a;background-size:cover;background-position:center;box-shadow:0 2px 12px rgba(0,0,0,0.15);transition:transform 0.2s,box-shadow 0.2s;cursor:pointer',
    overlay: 'position:absolute;inset:0;background:linear-gradient(to top,rgba(0,0,0,0.85) 0%,rgba(0,0,0,0.30) 55%,rgba(0,0,0,0.04) 100%);border-radius:inherit;pointer-events:none;z-index:0',
    inner: 'position:relative;z-index:1;display:flex;flex-direction:column;gap:5px;',
    tag: 'display:inline-flex;align-items:center;gap:5px;font-family:inherit;font-size:10px;font-weight:700;letter-spacing:0.09em;text-transform:uppercase;color:#fff;background:rgba(255,255,255,0.13);border:1px solid rgba(255,255,255,0.32);backdrop-filter:blur(4px);padding:3px 9px;border-radius:999px;width:fit-content',
    title: 'font-family:inherit;font-size:14px;font-weight:700;line-height:1.38;color:#fff;text-shadow:0 2px 8px rgba(0,0,0,0.75);margin:0;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden',
    meta: 'font-family:inherit;font-size:11px;font-weight:500;color:rgba(255,255,255,0.65);line-height:1.3',
    cta: 'display:inline-flex;align-items:center;gap:4px;margin-top:5px;font-family:inherit;font-size:11px;font-weight:700;letter-spacing:0.06em;color:#ffc800;text-decoration:none;width:fit-content'
  };
    ...
    fetch('/api/portal-posts?sort=popular&limit=10')
      ...
      .then(function(d){
        ...
        posts.forEach(function(p, i){
          ...
          var imgHtml = p.imagem ? '<img src="' + p.imagem + '" alt="" style="width:100%;height:100%;object-fit:cover;display:block;" loading="lazy" onerror="this.parentNode.style.background=\'' + cor + '\';this.remove();">' : '';
          ...
          html += '<a href="' + buildUrl(p) + '" style="display:flex;flex-direction:column;text-decoration:none;border-radius:10px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.10);transition:transform 0.18s,box-shadow 0.18s;" onmouseover="this.style.transform=\'translateY(-2px)\';this.style.boxShadow=\'0 6px 20px rgba(0,0,0,0.16)\'" onmouseout="this.style.transform=\'\';this.style.boxShadow=\'0 2px 8px rgba(0,0,0,0.10)\'">'
            + '<div style="position:relative;height:90px;background:' + (p.imagem ? '#0f172a' : cor) + ';flex-shrink:0;overflow:hidden;">'
              ...
              + '<div style="position:absolute;top:6px;left:6px;"><span style="' + (rank === 1 ? 'background:#e11d48;font-size:10px;' : 'background:rgba(0,0,0,0.6);font-size:9px;') + 'color:#fff;font-weight:900;padding:2px 7px;border-radius:4px;font-family:Georgia,serif;">#' + rank + '</span></div>'
            + '</div>'
